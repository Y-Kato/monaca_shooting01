...
class Enemy1 extends Enemy {
	...
	Shot() {
		if (g_game_status == g_status.GameOver)
			return;
		let x = g_owns.X - this.X;
		let y = g_owns.Y - this.Y;
		let angle1 = Math.atan2(y, x);
		let speed = 0.15 + 0.01 * g_stage_num;
		let burret = new EnemyBurret(this.X, this.Y, this.Z, Math.cos(angle1) * speed, Math.sin(angle1) * speed, 0);
		g_enemies_bullets.push(burret);
		burret.AddScene(g_scene);
	}
}
